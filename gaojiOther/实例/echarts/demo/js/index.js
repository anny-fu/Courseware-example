...
function dataLoad(json) {
  ...
  // 生成表头数据
  for (const x in json) {
    $wcThead.append(`<th>${json[x]['name']}</th>`);
  }
  // 创建表格中的第一列（周）
...
function sumExpense() {
  ...
  // 表脚的单元格
  let tfootTr = $weekConsumption.children("tfoot").children("tr");
  tbodyTd.each(function(bodyTdIdx, bodyTdItem) {
    ...
    if (bodyTdIdx !== 0) {
      ...
      tbodyTr.each(function(bodyTrIdx, bodyTrItem) {
        // 将当前列的数据添加进arr数组
        let currentVal = $(bodyTrItem).children("td").eq(bodyTdIdx).text();
        arr.push(currentVal);
      });
...
function inputRecalc(json) {
  ...
  $inputCell.on({
    ...
    blur: function() {
      ...
      if (thisVal === "") {
        $(this).text(0);
        $tfoot.find("td").eq(thisIdx).text('总计').removeClass("sumErro");
      }
      // 如果输入的值不全为数值
